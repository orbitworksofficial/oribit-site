...
import { ArrowUpRight, ExternalLink, Menu, Sparkles, X } from "lucide-react"
import { useState } from "react"
import { HeroSection } from "@/components/aeo/HeroSection"
import { WhatsAppMark } from "@/components/aeo/AiMarks"
...
/**
 * A real screenshot of an AI answer, framed as a browser window.
 *
 * These replace the hand-drawn mockups the design shipped with. The section
 * claims "real answer environments" — actual captures make that true, and they
 * carry each platform's own chrome and logo, which no approximation does.
 */
const PLATFORMS = [
  { id: "chatgpt", name: "ChatGPT", dot: "#10A37F" },
  { id: "gemini", name: "Gemini", dot: "#4C8DF6" },
  { id: "perplexity", name: "Perplexity", dot: "#20B8CD" },
  { id: "google", name: "Google AI Overview", dot: "#EA4335" },
  ...
  return (
    <figure className="shot">
      <figcaption className="shot-top">
        ...
        <span>{name}</span>
        <span className="shot-live" style={{ color: dot }}>●</span>
      </figcaption>
      <img
        src={`/ai/${id}.jpeg`}
        alt={`${name} answering a question about companies in an industry`}
        width={1200}
        height={630}
        loading="lazy"
        decoding="async"
...
export default function AeoGeoPage() {
  ...
  return <main className="aeo-page">
    ...
    <section className="platforms section"><div className="section-intro"><p className="eyebrow blue">04 / THE PLAYGROUND</p><h2>We make your brand<br/><span>legible to machines.</span></h2><p className="section-note">Real answer environments. Real opportunities to be the source.</p></div>{/*
      A marquee rather than a grid. Five 1200x630 captures tiled three-up render
      too small to read the answer text, which is the whole point of showing
